...
class getStartedStudent extends StatefulWidget {
  const getStartedStudent({Key? key}) : super(key: key);

  @override
  State<getStartedStudent> createState() => _getStartedStudentState();
}

class _getStartedStudentState extends State<getStartedStudent> {
  bool isHide = false;
                                    ...
                                    child: IntroductionScreen(
                                      ...
                                      pages: [
                                        ...
                                        PageViewModel(
                                            ...
                                            bodyWidget: Row(
                                              children: [
                                                mainTextFAQS("Select your School or institute", secColor, 20.0, FontWeight.bold, 1),
                                                Spacer(),
                                              ],
                                            ),
                                            footer:  enterSchool(),
                                        ...
                                        PageViewModel(
                                            titleWidget: Row(
                                              children: [
                                                mainTextFAQS("Select your grade", textColor, 35.0, FontWeight.bold, 1),
                                                Spacer(),
                                              ],
                                            ),
                                            bodyWidget: Row(
                                              children: [
                                                mainTextFAQS("Select your Grade", secColor, 20.0, FontWeight.bold, 1),
                                                Spacer(),
                                              ],
                                            ),
                                            footer:  enterClass(),
  ...
  Future<void> _launchUrl(String _url) async {
    ...
    if(_introKey.currentState!.getCurrentPage() == 8){
      if(stateofPhoto.img != ""){
        setState((){
          isHide = true;
        });
  ...
  postData(String imgs) async {
    ...
    if(response.statusCode == 200){
      Map ud = jsonDecode(response.body);
      postDatachild(imgs, ud['user'].toString());
    }
    else{
      setState(() {
        isHide = false;
  ...
  postDatachild(String imgs,String id) async {
    setState(() {
      isHide = true;
    });
    ...
    if(response.statusCode == 200){
      ...
      toaster(context,"User Created Successfully");
    }
    else{
      setState(() {
        isHide = false;
  ...
  next(){
    ...
    else if(_introKey.currentState!.getCurrentPage() == 6){
      if(stateofenterMacro.formKey.currentState!.validate()){
        _introKey.currentState!.next();
      }
    }
    else if(_introKey.currentState!.getCurrentPage() == 7){
